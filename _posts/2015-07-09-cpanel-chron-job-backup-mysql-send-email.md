...
Set your time interval to a few minutes from whatever time it is now so you can test that it is working. It will store the backup file temporarily on your server and then delete once the email has been sent. (with the g-zipped database as an attachment)
